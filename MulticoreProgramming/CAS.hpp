...
namespace CAS {
	...
	template <class T> concept AtomicablePointer = is_atomicable<T>::value && is_pointer<T>::value;

	// V should be present within tagmask bits.
	template <AtomicablePointer T, class V>
	inline bool CASEx(volatile T& targetPtr, T expectedPtr, T newValuePtr
...
#endif // _WIN64
		constexpr size_t pointmask = (SIZE_MAX ^ tagmask);
		...
		bool ret = CAS(compressedTarget, compressedExpected, compressedNew);
		targetPtr = reinterpret_cast<T>(compressedTarget & pointmask);
